...
LABEL maintainer="Jayaprakash N<jprakash.ora@gmail.com>"
ARG NB_USER="root"
# ARG NB_UID="1000"
ARG NB_GID=0
...
RUN \
    ...
    # Required later to use command add-apt-repository
...
RUN \
    ...
    apt-get install -y --no-install-recommends \
        ...
        libarchive-tools \
        # zlibc \ # obsolete and removed from Debian
...
#     mkdir -p $CONDA_ROOT && \
#     chown "${NB_USER}:${NB_GID}" "${CONDA_ROOT}" && \
...
# ENV \
    ...
    # MINICONDA_MD5=32d73e1bc33fda089d7cd9ef4c1be542616bd8e437d1f77afeeaf7afdb019787 \
...
# Setting the path here is important for mamba to work
...
# Replacing Minicoda with micromamba & mamba
COPY --chown="${NB_UID}:${NB_GID}" initial-condarc "${CONDA_ROOT}/.condarc"
WORKDIR /tmp
RUN set -x && \
    arch=$(uname -m) && \
    if [ "${arch}" = "x86_64" ]; then \
   ...
   mamba list python | grep '^python ' | tr -s ' ' | cut -d ' ' -f 1,2 >> "${CONDA_ROOT}/conda-meta/pinned" && \
   mamba clean --all -f -y && \
   fix-permissions.sh "${CONDA_ROOT}" && \
   clean-layer.sh
...
RUN \
    ...
    wget -qO- http://deb.nodesource.com/setup_16.x |sed 's/https:\/\//http:\/\//g' | sudo -E bash - && \
    apt-get --allow-unauthenticated install -y nodejs && \
...
## Glances webtool is installed in python section below via requirements-minimal.txt

## GPUView
COPY resources/tools/gpuview.sh $RESOURCES_PATH/tools/gpuview.sh
RUN \
    /bin/bash $RESOURCES_PATH/tools/gpuview.sh --install && \
    # Cleanup
    clean-layer.sh
...
RUN \
    ...
    if [ "$WORKSPACE_FLAVOR" = "minimal" ]; then \
        ...
        mamba install -y --update-all 'python='$PYTHON_VERSION nomkl ; \
    else \
...
RUN \
    ...
    # Remove Installation from requirements.txt
    # TODO: Enable and Activate qgrid
    # jupyter nbextension enable --py --sys-prefix qgrid && \
    # TODO: Activate Colab support
    # TODO: Install & activate GOTO error
    # https://github.com/teticio/nbextension-gotoerror
...
RUN \
    ...
    # Fix or install jupyterlab/toc extension
    #$lab_ext_install @jupyterlab/toc && \
    # jupyterlab/toc is now part of core jupyterlab
    ...
    # Install dependencies for jupyterlab_code_formatter
    pip install black isort yapf autopep8 && \
    #Fix enabling jupyterlab_code_formatter
...
RUN \
    ...
    VS_JUPYTER_VERSION="2022.7.1002081042" && \
    ...
    VS_PYTHON_VERSION="2023.13.12011006" && \
    #wget --no-verbose https://github.com/microsoft/vscode-python/releases/download/$VS_PYTHON_VERSION/ms-python-release.vsix && \
    wget --no-verbose https://marketplace.visualstudio.com/_apis/public/gallery/publishers/ms-python/vsextensions/python/$VS_PYTHON_VERSION/vspackage -O ms-python-release.vsix && \
...
LABEL \
    "maintainer"="jprakash0205@gmail.com" \
